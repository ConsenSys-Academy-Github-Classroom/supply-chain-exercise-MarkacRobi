...
contract SupplyChain {
  ...
  modifier forSale (uint sku) {
    require(items[sku].seller != address(0) && items[sku].state == State.ForSale);
    _;
  }

  modifier sold (uint sku) {
    require(items[sku].seller != address(0) && items[sku].state == State.Sold);
    _;
  }

  modifier shipped (uint sku) {
    require(items[sku].seller != address(0) && items[sku].state == State.Shipped);
    _;
  }
  // modifier received(uint _sku)
  modifier received (uint sku) {
    require(items[sku].seller != address(0) && items[sku].state == State.Received);
    _;
  }
